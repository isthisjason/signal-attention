...
The name comes from the paper *Attention Is All You Need*. That paper made sequence modeling feel concrete to me: instead of treating data points as isolated rows, the model attends to relationships across a sequence. SignalAttention is not a production Transformer trading model, but that idea shaped the project. I wanted to build the backend and simulation foundation first, then add attention based market regime experiments in a way that stayed honest and reproducible.

This is not a trading bot. It does not connect to brokers, place real orders, manage money, or give investment advice. The point is to explore strategy behavior, risk scoring, simulation, and ML analysis inside a local app I can run, inspect, and explain.
...
The ML side of this project is intentionally cautious. I did not want to build a black box price predictor and pretend it could tell someone what to buy. The safer and more useful question is: given a strategy run and recent candles, does this look fragile, risky, unusual, or dependent on the current market regime?
...
Frontend render and state tests were verified locally on May 31, 2026 during the Phase 7 polish pass. The full backend, ML service, frontend build, and smoke helper checks should be rerun before recording final demo evidence. Compose and the running stack smoke demo were not rerun in the current WSL distro because Docker CLI is unavailable here.

The next work I would do is mostly final evidence capture: verify the full Compose demo in a Docker enabled environment, save the smoke output, and capture the dashboard and Swagger screenshots listed in the docs.
